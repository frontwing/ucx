...
#define UCP_COLL_STEP_IDX_IS_PENDING(step, idx) ((step)->pending & UCS_BIT(idx))
#define UCP_COLL_STEP_IDX_UPDATE_PENDING(step, idx) (step)->pending &= ~UCS_BIT(idx)

struct ucp_coll_step_req {
	ucp_request_t    req;
	ucp_coll_step_t *step;
};

static UCS_F_ALWAYS_INLINE void
ucp_coll_comp_request_completed(ucp_request_t *req, ucs_status_t status)
{
...
static UCS_F_ALWAYS_INLINE void
ucp_coll_comp_common_check(ucp_coll_step_t *step, ucs_status_t status)
{
	/* Check the status, possibly invoking the fault-tolerance callback */
	ucp_coll_op_t *op = step->op;
...
static UCS_F_ALWAYS_INLINE void
ucp_coll_comp_step_cb(ucp_coll_step_t *step, ucs_status_t status)
{
	/* trigger the next step, if it exists */
	ucp_coll_op_t *op = step->op;
	ucs_assert(step == ucs_queue_head_elem_non_empty(&op->step_q, ucp_coll_step_t, queue));
	...
	if (ucs_queue_is_empty(&op->step_q)) {
		...
		ucs_assert(request->flags & UCP_REQUEST_FLAG_COLLECTIVE);
		ucp_coll_comp_request_completed(request, status);
	    op->cb_req = NULL; /* Mark op as ready again */
	} else {
		/* Start on the next step for this collective operation */
		ucp_coll_step_execute(ucs_queue_head_elem_non_empty(&op->step_q,
				ucp_coll_step_t, queue));
	}
}

static void ucp_coll_comp_send_one_cb(void *cb_data, ucs_status_t status)
{
	ucp_request_t *request = (ucp_request_t*)cb_data - 1;
	ucp_coll_step_req_t *step_comp =
			ucs_container_of(request, ucp_coll_step_req_t, req);
	ucp_coll_step_t *step = step_comp->step;
    ucp_coll_comp_common_check(step, status);
    ucp_coll_comp_step_cb(step, status);
}

static void ucp_coll_comp_recv_one_cb(void *cb_data, ucs_status_t status, ucp_tag_recv_info_t *info)
{
	ucp_coll_comp_send_one_cb(cb_data, status);
}

static void ucp_coll_comp_send_many_cb(void *cb_data, ucs_status_t status)
{
	ucp_request_t *request = (ucp_request_t*)cb_data - 1;
	ucp_coll_step_req_t *step_comp =
			ucs_container_of(request, ucp_coll_step_req_t, req);
    ...
    ucp_coll_comp_common_check(step, status);
    ucs_assert(UCP_COLL_STEP_IDX_IS_PENDING(step, cb_idx));
    UCP_COLL_STEP_IDX_UPDATE_PENDING(step, cb_idx);
    if (step->pending == 0) {
        ucp_coll_comp_step_cb(step, status);
    }
}

static void ucp_coll_comp_recv_many_cb(void *cb_data, ucs_status_t status, ucp_tag_recv_info_t *info)
{
	ucp_coll_comp_send_many_cb(cb_data, status);
}

static void ucp_coll_comp_reduce_one_cb(void *cb_data, ucs_status_t status, ucp_tag_recv_info_t *info)
{
	ucp_request_t *request = (ucp_request_t*)cb_data - 1;
	ucp_coll_step_req_t *step_comp =
			ucs_container_of(request, ucp_coll_step_req_t, req);
	...
	if (status == UCS_OK) {
		ucp_group_collective_params_t *params = &step->op->params;
		(void) mpi_reduce(params->cb_r_op,
				((ucp_coll_step_req_t*)request)->req.recv.buffer,
...
static void ucp_coll_comp_reduce_many_cb(void *cb_data, ucs_status_t status, ucp_tag_recv_info_t *info)
{
	ucp_request_t *request = (ucp_request_t*)cb_data - 1;
	ucp_coll_step_req_t *step_comp =
			ucs_container_of(request, ucp_coll_step_req_t, req);
...
static ucs_status_t ucp_coll_step_select_callbacks(ucp_coll_topo_phase_t *phase,
		ucp_send_callback_t *send_cb, ucp_tag_recv_callback_t *recv_cb, int nonzero_length)
{
	switch (phase->method) {
	case UCP_COLL_TOPO_METHOD_SEND:
		*send_cb = (phase->ep_cnt > 1) ? ucp_coll_comp_send_many_cb : ucp_coll_comp_send_one_cb;
		break;

	case UCP_COLL_TOPO_METHOD_REDUCE:
		*recv_cb = (phase->ep_cnt > 1) ? ucp_coll_comp_reduce_many_cb : ucp_coll_comp_reduce_one_cb;
		if (nonzero_length) break; /* otherwise no reduction needed! */

	case UCP_COLL_TOPO_METHOD_REDUCE_RECURSIVE:
		*send_cb = ucp_coll_comp_send_many_cb;
		*recv_cb = ucp_coll_comp_reduce_many_cb;
		if (nonzero_length) break; /* otherwise no reduction needed! */

	case UCP_COLL_TOPO_METHOD_RECV:
		*recv_cb = (phase->ep_cnt > 1) ? ucp_coll_comp_recv_many_cb : ucp_coll_comp_recv_one_cb;
		break;

	case UCP_COLL_TOPO_METHOD_STABLE_REDUCE:
	case UCP_COLL_TOPO_METHOD_STABLE_REDUCE_RECURSIVE:
...
static ucs_status_t ucp_coll_am_short_cb_many_wrapper(uct_pending_req_t *self) {
	...
	ucp_coll_comp_send_many_cb(req + 1, status);
	return status;
}

static ucs_status_t ucp_coll_am_short_cb_one_wrapper(uct_pending_req_t *self) {
    ucp_request_t *req = ucs_container_of(self, ucp_request_t, send.uct);
	ucs_status_t status = uct_ep_am_short(ucp_ep_get_am_uct_ep(req->send.ep),
			UCP_AM_ID_EAGER_ONLY, req->send.tag.tag, req->send.buffer, req->send.length);
	ucp_coll_comp_send_one_cb(req + 1, status);
	return status;
}

static ucs_status_t ucp_coll_tag_eager_short_cb_many_wrapper(uct_pending_req_t *self) {
    ucp_request_t *req = ucs_container_of(self, ucp_request_t, send.uct);
...
static ucs_status_t ucp_coll_tag_eager_short_cb_one_wrapper(uct_pending_req_t *self) {
    ucp_request_t *req = ucs_container_of(self, ucp_request_t, send.uct);
	ucs_status_t status = uct_ep_tag_eager_short(ucp_ep_get_tag_uct_ep(req->send.ep),
			req->send.tag.tag, req->send.buffer, req->send.length);
	ucp_coll_comp_send_one_cb(req + 1, status);
...
static UCS_F_ALWAYS_INLINE void
ucp_coll_step_create_send_req(ucp_ep_h ep, ucp_request_t *req,
							  ...
							  ucp_group_collective_params_t *params)
{
	...
	/* TODO: use...
	int is_data_split = (step->flags & UCP_COLL_STEP_FLAG_LENGTH_PER_REQUEST);
	if (is_data_split) {
	...
	*/
	...
	if ((ssize_t)length <= ucp_ep_config(ep)->tag.max_eager_short) {
		if (cb == ucp_coll_comp_send_many_cb) {
			req->send.uct.func = ucp_coll_am_short_cb_many_wrapper;
		} else {
			ucs_assert(cb == ucp_coll_comp_send_one_cb);
			req->send.uct.func = ucp_coll_am_short_cb_one_wrapper;
		}
	} else if ((ssize_t)length <= ucp_ep_config(ep)->tag.offload.max_eager_short) {
		if (cb == ucp_coll_comp_send_many_cb) {
			req->send.uct.func = ucp_coll_tag_eager_short_cb_many_wrapper;
		} else {
			ucs_assert(cb == ucp_coll_comp_send_one_cb);
			req->send.uct.func = ucp_coll_tag_eager_short_cb_one_wrapper;
		}
	} else {
...
static UCS_F_ALWAYS_INLINE void
ucp_coll_step_create_recv_req(ucp_ep_h ep, ucp_request_t *req,
...
ucp_coll_step_execute_recv(ucp_request_t *req)
{
...
ucs_status_t ucp_coll_step_create(ucp_coll_topo_phase_t *phase,
		ucp_group_collective_params_t *params,
		...
		ucp_coll_step_req_t **cb_ptr)
{
	...
	ucs_status_t status = ucp_coll_step_select_callbacks(phase,
	...
	step->flags        = 0; // TODO: set UCP_COLL_STEP_FLAG_LENGTH_PER_REQUEST when needed
	step->send_req_cnt = 0;
	step->recv_req_cnt = 0;
	step->reqs         = *cb_ptr;
	...
	unsigned ep_idx;
	ucp_coll_step_req_t *req = step->reqs;
	ucp_ep_h ep, *eps = UCP_COLL_GET_EPS(phase);
	for (ep_idx = 0, ep = *eps; ep_idx < phase->ep_cnt; req++, ep_idx++, ep = *(eps++)) {
		req->step = step;
		switch (phase->method) {
		case UCP_COLL_TOPO_METHOD_SEND:
			ucp_coll_step_create_send_req(ep, &req->req, send_cb, params);
			step->send_req_cnt++;
			break;
		...
		case UCP_COLL_TOPO_METHOD_REDUCE:
		case UCP_COLL_TOPO_METHOD_STABLE_REDUCE:
			ucp_coll_step_create_recv_req(ep, &req->req, recv_cb, params);
			step->recv_req_cnt++;
			break;
...
ucs_status_t ucp_coll_step_execute(ucp_coll_step_t *step)
{
    ...
    ucs_assert(step == ucs_queue_head_elem_non_empty(&step->op->step_q, ucp_coll_step_t, queue));
    ...
    for (req_idx = 0; req_idx < step->recv_req_cnt; req_idx++, req++) {
    	ucs_status_t status = ucp_coll_step_execute_recv(&req->req);
    	if (ucs_unlikely(UCS_STATUS_IS_ERR(status))) {
    		return status;
    	}
    }
		...
		ucp_coll_op_t **new_op)
{
	...
	ucp_coll_op_t *op = ucs_malloc(sizeof(ucp_coll_op_t) + topo->phs_cnt *
			(sizeof(ucp_coll_step_t) + 2 * topo->ep_cnt * sizeof(ucp_coll_step_req_t)),
			"ucp_coll_op");
	if (!op) {
	...
	ucp_coll_step_t *next_step = &op->steps[0];
	ucp_coll_topo_phase_t *next_phase = &topo->phss[0];
	ucp_coll_step_req_t *cb_ptr = (ucp_coll_step_req_t*)&op->steps[topo->phs_cnt];
	for (op->step_cnt = 0;
		 op->step_cnt < topo->phs_cnt;
		 op->step_cnt++, next_step++, next_phase++) {
		ucs_status_t ret = ucp_coll_step_create(next_phase,
	...
	*new_op = ucs_realloc(op, (char*)cb_ptr - (char*)op, "ucp_coll_op");
