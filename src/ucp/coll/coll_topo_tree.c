...
static inline ucs_status_t ucp_coll_topo_connect_phase(ucp_coll_topo_phase_t *phase,
        ...
        ucp_group_member_index_t *peers, unsigned peer_cnt,
        enum ucp_coll_topo_method_type method)
{
    ...
    while (status == UCS_OK && peer_idx) {
        ucp_group_member_index_t next_peer = peers[--peer_idx];
...
static inline ucs_status_t ucp_coll_topo_connect_tree(ucp_coll_topo_t *tree,
        ...
        ucp_group_member_index_t *up, unsigned up_cnt,
        ucp_group_member_index_t *down, unsigned down_cnt)
{
    ucs_status_t status          = UCS_OK;
...
static ucs_status_t ucp_coll_topo_tree_add_net(struct ucp_coll_topo_params *params,
        ucp_group_member_index_t my_idx,
        ucp_group_member_index_t *up, unsigned *up_cnt,
        ucp_group_member_index_t *down, unsigned *down_cnt)
{
    /**
    ...
    for (peer_idx = 0; peer_idx < *down_cnt; peer_idx++) {
        ...
        case UCP_GROUP_MEMBER_DISTANCE_SOCKET:
        ...
        case UCP_GROUP_MEMBER_DISTANCE_HOST:
    ...
    ucp_group_member_index_t range_size, member_idx;
    ucp_group_member_index_t radix = params->tree_radix;
...
{
    ucp_group_member_index_t my_idx;
    ucp_group_member_index_t member_idx;
    ucp_group_member_index_t up[MAX_PEERS];
    ucp_group_member_index_t down[MAX_PEERS];
    unsigned up_cnt = 0, down_cnt = 0;
    enum ucp_group_member_distance up_distance = UCP_GROUP_MEMBER_DISTANCE_LAST;
    enum ucp_group_member_distance down_distance = UCP_GROUP_MEMBER_DISTANCE_SELF;
     ...
     */
    enum ucp_group_member_distance master_phase = UCP_GROUP_MEMBER_DISTANCE_NET;
    ...
    for (member_idx = 0; member_idx < params->group_params->member_count; member_idx++) {
        enum ucp_group_member_distance next_distance =
                params->group_params->distance[member_idx];

        /* Possibly add the next member to my list according to its distance */
        if (ucs_unlikely(next_distance == UCP_GROUP_MEMBER_DISTANCE_SELF)) {
            seen_myself = 1;
            my_idx = member_idx;
        } else if (seen_myself) {
            ...
            if ((next_distance <= master_phase) &&
                ...
                (next_distance != UCP_GROUP_MEMBER_DISTANCE_NET)) {
    ...
    if (master_phase >= UCP_GROUP_MEMBER_DISTANCE_HOST) {
